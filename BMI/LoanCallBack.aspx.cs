...
public partial class BMI_LoanCallBack : System.Web.UI.Page
{
                ...
                if (trnsactionInfo != null)
                {
                    ...
                    switch (Request.Params["ResCode"])
                    {
                        case "0":
                            ...
                            {
                                byte status = 0;
                                BMI.PayLoan(trnsactionInfo.AccountNo, trnsactionInfo.Amount, saleReferenceId, ref status);
                                if (status == 0)
                                {
